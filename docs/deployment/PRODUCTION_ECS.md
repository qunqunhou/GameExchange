...
### 1.5 P3.2-C/D/E ECS 部署、私有验收与配置收敛记录

验证日期：`2026-08-09`。Overall Status：`PRIVATE_VALIDATION_PASSED`。本次在部署前完成在线配置、挂载、Secret、回滚镜像和数据库备份审计，随后只替换 App 容器，通过 SSH Tunnel 完成无域名私有验收，并在不重建容器的前提下收敛基础 `APP_IMAGE`。
